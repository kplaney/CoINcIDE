  ...
  if(!is.null(esetList)){
    ...
    if(validObject(esetList[[1]])){
      ...
      for(d in 1:length(esetList)){
        ...
        if(d>1){
          ...
          phenoMasterDF <- pData(esetList[[d]])
    ...
    for(d in 1:length(dataMatrixList)){
      ...
      if(d>1){
        ...
        phenoMasterDF <- dataMatrixList[[d]]$phenoData
...
# #COME BACK: try GSEAbase.
# GSEA <- function(testGeneVector,refGeneLists,method=c("hypergeometric","fisher"),genomeSize,
...
#   
#   
#   output <- list(qvalues=qvalues,pvalues=unlist(pvalues));
#   
#   return(output);
#   
# }
# 
# #try a heatmap of ranks (color by community)?
# #then also a simple heatmap of median ranks for each meta-community
# 
# mergedCommAndClinicalData <- function(sampleClustCommKey,masterClinicalDF,esetList, mergeSampleIDCol=""){
#   
#   
#   if(!missing(esetList)){
#     
