...
selectFeaturesMetaVariance_wrapper(dataMatrixList,rankMethod=c("sd,MAD"), 
                                   numNAstudiesAllowedPerFeat=ceiling(length(dataMatrixList)/10),numFeatSelect=1000,
  ...
  output <- selectFeaturesMetaVariance(rankMatrix=rankMatrix,numNAstudiesAllowedPerFeat=numNAstudiesAllowedPerFeat,numFeatSelect=numFeatSelect,
...
computeFeatureRanks <- function(dataMatrixList,method=c("sd,MAD")){
  ...
  outputList <- lapply(dataMatrixList,FUN=function(dataMatrix,method){
    
    if(method=="sd"){
      
      output <- rowSds(dataMatrix)
      
    }else if(method=="MAD"){
      
      output <- rowMads(dataMatrix)
      
    }
    
  names(output) <- rownames(dataMatrix)
  tmp <- sort(output,decreasing = TRUE)
  ...
  names(outputList) <- names(dataMatrixList)
...
selectFeaturesMetaVariance <- function(rankMatrix,numNAstudiesAllowedPerFeat=ceiling(ncol(rankMatrix)/10),numFeatSelect=1000,
  ...
  #count up # non-NAs across studies
  ...
  #don't take features that were only in a few datasets
  ...
  #want small ones (higher mean ranks) to be at the top.
  metric <- sort(metric,decreasing=FALSE)[c(1:numFeatSelect)]
  ...
  cat(paste0(length(topFeatures)," features from numNAstudiesAllowedPerFeat= ",numNAstudiesAllowedPerFeat," threshold\n",
             "and ",numFeatSelect, " from general meta-ranking for a total of ",length(uniqueFeat)," unique features.\n"),append=TRUE,file=outputFile)

  rankMatrix <- rankMatrix[uniqueFeat, ]
  ...
  output <- list(finalRankMatrix=rankMatrix,finalFeatures=uniqueFeat,topFeaturesAcrossDatasets=topFeatures,
                 datasetsToRemove=datasetsRemove)
