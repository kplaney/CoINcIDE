...
experimentName <- "pam50Centroid_pear_meanCent_MM75"
...
#if want to control y-axis:

#densityPlot <- meanMetricDensityPlot(CoINcIDE_output$meanMetricMatrix,saveDir=saveDirPAM50,
#                                    experimentName=experimentName,savePlot=TRUE,yLimit=2.5)
#if want to set y-axis. also try density without non-optimal matches:
tmpMatrix <- CoINcIDE_output$meanMetricMatrix
tmpMatrix[which(is.na(CoINcIDE_output$pvalueMatrix))] <- NA
densityPlot <- meanMetricDensityPlot(tmpMatrix,saveDir="~/",
                                     experimentName=paste0(experimentName,"onlyOptimal"),savePlot=TRUE,yLimit=2.5)
...
breast_pam50FullCentroidsOut <- metaFeaturesAnalysisWrapper(metaFeatures=metaFeatures,esets=esets,CoINcIDE_output=CoINcIDE_output , clusterCoINcIDE_output=clusterCoINcIDE_output,
                                                            meanEdgePairPvalueThresh = .01,indEdgePvalueThresh = .01, minTrueSimilThresh = .75, maxTrueSimilThresh = Inf,minFractNN=.8,
                                                            ...
                                                            findCommWithWeights=TRUE, plotSimilEdgeWeight = FALSE,clinVarPlots=TRUE,plotToScreen=FALSE,fractEdgesInVsOutEdge=0,
...
densityPlot <- meanMetricDensityPlot(CoINcIDE_output$meanMetricMatrix,saveDir="~/",
                                     experimentName=experimentName,savePlot=TRUE,yLimit=2.5)
...
experimentName <- "pam50FullKmeans_pear_meanCent_MM5"
eset_featureDataFieldName="gene_symbol"
networkColors = "Set3"
outcomesVarBinary=NA
#not enough continuous variables for breast data
...
tmpMatrix <- CoINcIDE_output$meanMetricMatrix
 tmpMatrix[which(is.na(CoINcIDE_output$pvalueMatrix))] <- NA
densityPlot <- meanMetricDensityPlot(tmpMatrix,saveDir="~/", experimentName=experimentName,savePlot=TRUE,yLimit=2.5
  experimentName=paste0(experimentName,"onlyOptimal"),savePlot=TRUE,yLimit=2.5)
