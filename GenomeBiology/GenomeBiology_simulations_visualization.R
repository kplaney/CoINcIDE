...
for(s in 1:length(simTypes)){
  ...
  if(s==1){
    ...
    #masterDF_sd <- data.frame(data$ROC_SDmatrixFull,rep(simTypes[s],nrow(data$ROC_SDmatrixFull)));
    #colnames(masterDF_sd)[ncol(masterDF_sd)] <- "sim_type";
    
  }else{
    
    tmp <- data.frame(data$ROC_matrixFull,rep(simTypes[s],nrow(data$ROC_matrixFull)));
    colnames(tmp)[ncol(tmp)] <- "sim_type";
    masterDF <- rbind(masterDF,tmp);
    
    #tmp <- data.frame(data$ROC_SDmatrixFull,rep(simTypes[s],nrow(data$ROC_SDmatrixFull)));
    #colnames(tmp)[ncol(tmp)] <- "sim_type";
    #masterDF_sd <- rbind(masterDF_sd,tmp);
...
colorCodes <- brewer.pal(length(simTypes),"Dark2");

names(colorCodes) <- simTypes
...
#it's zero for this simil level:
FPR_plot_color <- ggplot(data =masterDF,aes(x=sd_noise,y=FPR,group=sim_type, colour=sim_type))  + geom_line(size=.7)+ 
  labs(title = "Network edge FPR for simulations with increasing noise",y="FPR",x="sd of additive random normal noise")+
  scale_color_manual(values=colorCodes)+
  theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  theme(axis.text.x = element_text(colour = "black",size=18),axis.title.x = element_text(colour = "black",size=20,vjust=0),
        axis.text.y = element_text(colour = "black",size=18),axis.title.y = element_text(colour = "black",size=20,vjust=1),
...
options(bitmapType="cairo")
png(filename=paste0(saveDir,"/FPR_plot_color_",meanSimil,"_",Sys.Date(),".png"),width=1400,height=1600,res=200);
...
options(bitmapType="cairo")
png(filename=paste0(saveDir,"/TPR_plot_",meanSimil,"_",Sys.Date(),".png"),width=1400,height=1600,res=200);
...
FPR_plot_point <- ggplot(data =masterDF,aes(x=sd_noise,y=FPR,group=sim_type))  + geom_line(size=.4)+ geom_point(aes(shape=factor(sim_type)),size=5)+
  labs(title = "",y="False positive rate",x="Noise level")+
  scale_color_manual(values=colorCodes)+
  theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  theme(axis.text.x = element_text(colour = "black",size=18),axis.title.x = element_text(colour = "black",size=20,vjust=0),
        axis.text.y = element_text(colour = "black",size=18),axis.title.y = element_text(colour = "black",size=20,vjust=1),
...
options(bitmapType="cairo")
png(filename=paste0(saveDir,"/FPR_plot_",meanSimil,"_",Sys.Date(),".png"),width=1400,height=1600,res=200);
