...
#NOTE: used to be named createExpressionSetList
createExprSetList <- function(exprMatrixList,masterPhenoData,patientKey="GEO_GSMID",
  ...
  for(d in 1:length(exprMatrixList)){
    ...
    if(!is.matrix(exprMatrixList[[d]]$expr)){
      ...
      stop("Error: this code assumes your expression data in the expr list index is a matrix, 
...
procExprSetList <- function(exprSetList,outputFile="./procExprSetMessages.txt",
  ...
  for(e in 1:length(exprSetList)){
    ...
    if(!missing(numTopVarGenes)){
     ...
     exprSetList[[e]] <- procExprSet( exprSetList[[e]],
      ...
      exprSetList[[e]] <- procExprSet( exprSetList[[e]],
                          outputFile=outputFile,minVar=minVar,featureDataFieldName=featureDataFieldName,uniquePDataID=uniquePDataID)
      
      
   }else{
      
      exprSetList[[e]] <- procExprSet(exprSetList[[e]],
                          outputFile=outputFile,featureDataFieldName=featureDataFieldName,uniquePDataID=uniquePDataID)
...
procExprSet <- function(exprSet,outputFile="./procExprSetMessages.txt",
  ...
  tmp <- filterAndImputeSamples(study,studyName = "study",
         outputFile = outputFile,impute=TRUE, 
...
filterAndImputeSamples <- function(study,studyName = "study",outputFile="./filterAndImputeMessagest.txt",
...
samples are now the columns for an pxn matrix.")
  ...
  message("Note: this function assumes your missing values
  ...
  cat("\nThere were ",length(goodGeneIndices), " genes that passed your max", 
  ...
  cat("\nThere were ",length(goodIndices), "patients that passed your max", 
  ...
  if(impute){
    ...
    if(numNAgenes > 0 && returnErrorRate){
      ...
      cat("\nThe average absolute error for imputation for study ", studyName,
...
file=outputFile,append=TRUE)
  ...
  if(missing(classIndex)){
    ...
    cat("\nFinished imputing study ",studyName, "\n",file=outputFile,append=TRUE)
...
This function just removes any genes whose key is NA.")
  ...
  message("\nYou may get a warning because key names are duplicated 
...
removeDupPatients <- function(exprMatrix, 
  ...
  if(length(duplicated_patients)>0){
    ...
    cat("\nFound duplicated samples for patients ",duplicated_patients,
        "patients. There may be more than 2 samples per patient.\n",
        append=TRUE,file=outputFile)
...
filterGenesByVariance <- function(study, plotSaveDir="~/",minVarPercentile,
                                  ...
                                  exprIndex = "expr", keysIndex = "keys", 
                                  outputFile="./filterByVarCalcOutput.txt",
...
mergeDatasetList <- function(datasetList,minNumGenes = 10000, minNumPatients = 40,batchNormalize = c('BMC','none','combat'),NA_genesRemove=TRUE,
                              outputFile="./mergeDatasets.txt",combatPvalueThresh=.05){
